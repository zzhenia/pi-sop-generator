...
_env = {
    "PI_CONFLUENCE_EMAIL": os.environ.get("PI_CONFLUENCE_EMAIL", ""),
    "PI_CONFLUENCE_API_TOKEN": os.environ.get("PI_CONFLUENCE_API_TOKEN", ""),
    "ANTHROPIC_API_KEY": os.environ.get("ANTHROPIC_API_KEY", ""),
}

PI = INSTANCES["pi"]
PI_AUTH = get_auth(_env, PI)
...
def get_config():
    return {
        ...
        "parent_folders": PI_PARENT_FOLDERS,
    }
...
def publish_sop(body: dict):
    ...
    try:
        storage_html = md_to_storage_format(markdown, metadata)
        space_id = find_space_id(PI_AUTH, PI["base"], PI["space_key"])
        page_id, page_url = create_page(
            PI_AUTH, PI["base"], space_id, title, storage_html, parent_id,
            draft=publish_as_draft,
        )
        add_label(PI_AUTH, PI["base"], page_id, "sop-metadata")
    except SystemExit as e:
